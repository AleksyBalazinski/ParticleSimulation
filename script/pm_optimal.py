...
field_s1 = load_data(r'output/field-s1.dat')

frames_s2 = load_data(r'output/positions-s2.dat')
field_s2 = load_data(r'output/field-s2.dat')
...
# Process datasets
r_s1, g_s1 = process_frame_data(frames_s1, field_s1, center=(30, 30, 15))
r_s2, g_s2 = process_frame_data(frames_s2, field_s2, center=(30, 30, 15))

# Compute expected and smoothed values for original
expected_g_x = G * m / r_s1**2
s1_g_x = s1_smoothed_g(r_s1, a)
s2_g_x = s2_smoothed_g(r_s2, a)
...
ax.plot(np.array(r_s1)/H, np.array(g_s1)/G, label='S1', linestyle="-", color="green", linewidth=1.5)
ax.plot(np.array(r_s2)/H, np.array(g_s2)/G, label='S2', linestyle="-", color="black", linewidth=1.5)
...
ax.plot(np.array(r_s1)/H, np.array(s2_g_x)/G, label='S2', linestyle="--", color="red", linewidth=1.5)
ax.set_title(r'$a = 4H$')
